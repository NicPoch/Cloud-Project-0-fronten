...
const Login=(props)=>{
    ...
    const submitForm=async(event)=>{
        ...
        if(!error){
            fetch("http://127.0.0.1:5000/login",{method:'POST',headers: {'Accept': 'application/json','Content-Type': 'application/json'},body:JSON.stringify(form)}).then(ans=>ans.json()).then(data=>{
                console.log(data);
                props.setUser(data);
